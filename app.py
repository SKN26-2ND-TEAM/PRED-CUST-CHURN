...
DESCRIPTION_KO = {
    ...
    "Reference date for features/label": "피처와 라벨을 산출한 기준일",
    "Customer region": "고객 거주 지역",
    "Customer age (years)": "고객 나이(연 단위)",
    "Age band bucket": "연령대 구간",
    "Marital status": "혼인 상태",
    "Tenure with insurer in months": "보험사와의 거래 기간(개월)",
    "1 if customer holds multiple policies, else 0": "복수 계약 보유 시 1, 아니면 0",
    "Number of policies held by customer (synthetic)": "고객이 보유한 계약 수(합성 데이터)",
    "Primary policy type": "주요 보험 상품 유형",
    "Policy renewal month (1-12)": "계약 갱신 월(1~12)",
    ...
    "Count of premium increases in last 3 years (synthetic)": "최근 3년 보험료 인상 횟수(합성 데이터)",
    ...
    "1 if missed payments flag (>=4 late payments), else 0": "납부 누락 위험 플래그(연체 4회 이상이면 1)",
    "1 if payment method changed recently, else 0": "최근 납부 방법 변경 시 1, 아니면 0",
    ...
    "Total payout divided by total claim amount (last 12 months)": "최근 12개월 총 청구 금액 대비 총 지급 금액 비율",
    "Average settlement time for claims (days)": "청구 평균 처리 기간(일)",
    "Days since last claim (proxy recency)": "마지막 청구 후 경과일",
    "Number of customer contacts in last 12 months": "최근 12개월 고객 접촉 횟수",
    "1 if complaint lodged, else 0": "민원 접수 시 1, 아니면 0",
    "Complaint resolution time in days (0 if no complaint)": "민원 처리 기간(민원 없으면 0일)",
    "1 if customer requested a quote (proxy shopping), else 0": "비교 견적 요청 시 1, 아니면 0",
    "1 if customer downgraded coverage, else 0": "보장 축소 시 1, 아니면 0",
    "Target label: 1 = churned at renewal, 0 = retained": "타깃 라벨: 갱신 시 이탈이면 1, 유지면 0",
    "Heuristic churn reason (synthetic)": "규칙 기반 이탈 사유(합성 데이터)",
    "Underlying churn probability used to generate labels (drop before upload if desired)": "라벨 생성에 사용한 내부 이탈 확률(업로드 전 제거 가능)",
}
...
def load_data():
    ...
    if DICT_PATH.exists():
        ...
        data_dict["description_ko"] = data_dict["description"].map(DESCRIPTION_KO).fillna(data_dict["description"])
        ...
        data_dict = data_dict[["컬럼명", "설명", "원문 설명"]]
